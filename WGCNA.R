...
library(limma)
library(WGCNA)
expFile="lncRNA_tpms_log2.txt"  

rt=read.table(expFile, header=T, sep="\t", check.names=F)
rt=as.matrix(rt)
#Calculate the mean value of duplicate genes
rownames(rt)=rt[,1]
exp=rt[,2:ncol(rt)]
dimnames=list(rownames(exp),colnames(exp))
data=matrix(as.numeric(as.matrix(exp)),nrow=nrow(exp),dimnames=dimnames)
data=avereps(data)
data=data[apply(data,1,sd)>0.8,]     

#Extract sample information
Type=gsub("(.*)\\-(.*)", "\\2", colnames(data))
conCount=length(Type[Type=="11A"])
treatCount=length(Type[Type=="01A"])
datExpr0=t(data)
...
traitData=data.frame(Con=c(rep(0,treatCount),rep(1,conCount)),
                     Treat=c(rep(1,treatCount),rep(0,conCount)))
...
###Average connectivity and power scatter plot
...
###Dynamic Shear Module Identification
minModuleSize =30    
...
###Cluster modules to identify similar module clusters
...
moduleTraitCor = cor(MEs, datTraits, use = "p")#相关性检验
...
###Calculate the MM and GS values
...
###Graphical representation of the importance of output modules
y=datTraits[,1]
...
###Batch output of traits and module scatter plots
trait="Treat"
...
###Output the core genes for each module
geneSigFilter=0.5       
moduleSigFilter=0.8      
